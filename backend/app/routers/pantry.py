...
def get_pantry(
    ...
    refresh_all_pantry_states(db, as_of)
    build_or_update_reminder_batch(db, as_of)
    statement = select(PantryState).options(joinedload(PantryState.item)).order_by(PantryState.item_id)
    if status != "all":
