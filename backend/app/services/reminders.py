...
def build_or_update_reminder_batch(db: Session, as_of: datetime | None = None) -> ReminderBatch | None:
    ...
    batch = db.scalar(
        ...
        .where(ReminderBatch.status == "pending")
    ...
    if batch is None:
        batch = ReminderBatch(status="pending", scheduled_for=scheduled_for)
        db.add(batch)
        db.flush()
    else:
