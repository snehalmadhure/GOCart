...
export const quantity = (value, unit) => value == null ? 'Not estimated' : `${value}${unit === '%' ? '%' : ` ${unit}`}`
export const daysLabel = (days) => !Number.isFinite(days) ? 'learning your pattern' : days <= 1 ? 'tomorrow' : days < 2 ? '1–2 days' : `~${Math.round(days)} days`
