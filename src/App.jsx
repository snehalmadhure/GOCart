...
function RequireAuth({ children }) {
 const { session, loading, isMockMode } = useAuth(); const location = useLocation()
 if (loading) return <main className="grid min-h-screen place-items-center text-slate-500">Loading your pantry…</main>
 if (!isMockMode && !session) return <Navigate to="/login" replace state={{ from: location }} />
 return children
}
...
export default function App() { return <Routes><Route path="/login" element={<LoginPage />} /><Route path="*" element={<RequireAuth><ProtectedApp /></RequireAuth>} /></Routes> }
